...
namespace
{
    ...
    Component *CreateTransform(GameObject *owner, const nlohmann::json &data)
    {
        ...
        {
            tr->SetPosition({data["position"][0], data["position"][1], data["position"][2]});
        }
        if (data.contains("rotation"))
        {
            Vector3 eulerAngles = {data["rotation"][0], data["rotation"][1], data["rotation"][2]};
            tr->SetOrientation(Quaternion::FromEulerAngles(eulerAngles));
        }
        if (data.contains("scale"))
        {
            tr->SetScale({data["scale"][0], data["scale"][1], data["scale"][2]});
        }
        return tr;
    }
